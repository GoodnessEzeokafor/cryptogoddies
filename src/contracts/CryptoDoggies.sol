...
contract CryptoDoggies is AccessControl, DetailedERC721{
   ...
   event TokenCreated(
    ...
    function _generateRandomDna() private view returns(bytes5){
        ...
        bytes5 hashVal = bytes32(block.blockhash(lastBlockNumber)); 
    ...
    function getAllTokens() public view returns(
        ...
        uint256[] memory owners = new address[](total);
        for(uint56 i = 0; i < total; i++){
    ...
    function purchase(uint256 _tokenId) public payable whenNotPaused{
        ...
        uint256 contractCut = sellingPrice.mul(6).divide(100);
    ...
    uint256 private increaseLimit4 = 5.02 ether;
